...
Status: complete, apart from the on-device verification, which is inherently manual. Notes/deviations:

- `vite-plugin-pwa@1.3.0` in `generateSW`/`autoUpdate` mode; manifest lives inline in `vite.config.ts`. Precache globs cover js/css/html/png only — soundfont samples come from a CDN at runtime and are deliberately not precached, so a cold offline launch opens the app but cannot sound until samples have been fetched once.
- Icons are generated by `scripts/make-icons.ts` (`npm run make-icons`) into `public/`: a hand-rolled PNG encoder over `node:zlib`, so the design is source rather than a checked-in binary and no image dependency is needed. Emits 192/512, a maskable 512 with a wider inset, and a 180 apple-touch-icon.
- `pwa/install.ts` splits the decision (`shouldShowInstall`, pure) from the environment probe (`installEnv(window, storage)`), so the logic is testable; `views/install.ts` is the screen, with iOS share-sheet steps and a generic fallback. Dismissal persists under `install:dismissed` and the screen is skipped entirely when already launched standalone.
- The install screen is mounted by `main.ts` ahead of the app and hides the placeholder nav; dismissing it starts the normal app. `registerSW({ immediate: true })` runs at the end of `main.ts`.
- `tsconfig.json` gained `vite-plugin-pwa/client` in `types` for the `virtual:pwa-register` module.
- Still outstanding: install to an iOS home screen, go offline, complete a session — together with the deferred audio/trial device checks.
