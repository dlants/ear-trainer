...
- **Touch, not click.** No hover-dependent affordances, no tooltips, no right-click, no tiny icon buttons. Use `pointerdown` for anything that should feel immediate (answer buttons) to skip the ~300ms click delay, and `touch-action: manipulation` to kill double-tap zoom. Every button gets a visible pressed state — for a young kid, confirmation that the tap registered is most of the interface.
  - The exception is controls that gate a browser capability requiring user activation — starting audio (`AudioContext.resume()`) and enabling the mic (`getUserMedia()`). Moving the start-audio button from `click` to `pointerdown` hung the unlock on Android with no resolve and no reject, so those controls stay on `click` and accept the slightly later feedback. Desktop Chromium with touch emulation and the Android emulator both grant activation on `pointerdown`, but the focused WebKit activation test demonstrates the intended boundary: a context resumed during touch `pointerdown` remains suspended until the tap completes, while a context resumed from `click` runs. That test proves the browser mechanism, not the exact Android Chrome failure.
