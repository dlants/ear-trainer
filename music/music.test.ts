...
test.describe("pattern identity", () => {
  test("keeps octaves distinct", () => {
    const ids = ["5-1", "5-1^", "5v-1"].map((s) => parse(s).id);
    expect(new Set(ids).size).toBe(3);
  });

  test("is scoped by context", () => {
    expect(parse("1-3-5").id).not.toBe(parse("1-3-5", "minor-cadence").id);
  });

  test("distinguishes #4 from b5 even though they sound the same", () => {
...
test.describe("parsing", () => {
  ...
  test("round-trips through the numeric formatter", () => {
    for (const input of [
      "1-2-3-4-5-6-7",
      "1+3+5-4+6",
      "♭3-♯4-♭7",
...
test.describe("noteToMidi", () => {
  ...
  test("voices a major cadence as I-IV-V-I", () => {
  ...
  test("voices a minor cadence as i-iv-V-i", () => {
